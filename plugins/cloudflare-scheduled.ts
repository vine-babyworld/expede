...
import { cronReconciliar, cronSyncPoll, cronSyncProdutosDiario, cronMLStatus, cronNfStatus, cronNfEmissao, cronRepasseMl, cronRepasseShopee } from "../src/server";
...
export default definePlugin((nitroApp) => {
  nitroApp.hooks.hook("cloudflare:scheduled", async ({ context }: any) => {
    // Estes crons disputam o mesmo limite de 3 req/s do Bling. Disparados em
    ...
    //
    // ESTA LISTA É A FONTE DA VERDADE dos crons em produção. O `scheduled` do
    // export default em src/server.ts não roda — não adicione cron lá.
    const tarefas: Array<[string, () => Promise<unknown>]> = [
      ["cron-sync", cronSyncPoll],
      ["cron-sync-produtos", cronSyncProdutosDiario],
